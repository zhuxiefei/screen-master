 ...
 * 物管App极光推送
 * </p>
 * ClassName: PropertyPushUtil <br/>
 * Author: Xia.yx  <br/>
 * Date: 2018/1/23 15:40 <br/>
 * Version: 1.0 <br/>
 */
public class PropertyPushUtil {
     ...
     * Date: 2017/8/8 9:37
     */
    private PropertyPushUtil() {
        if (LOG.isInfoEnabled()) {
            LOG.info("============jiguang init start...=========");
        }
        String appKey = ConfigManager.read((ConfigName.PROPERTY_APP_KEY));
        String masterSecret = ConfigManager.read((ConfigName.PROPERTY_MASTERSECRET));
        if (LOG.isInfoEnabled()) {
     ...
     * 单例对象，在系统启动时创建
     */
    private static PropertyPushUtil instance = new PropertyPushUtil();
     ...
     * @return 单例对象
     */
    public static PropertyPushUtil getInstance() {
        return instance;
    }
     ...
     * return PushPayload
     */
    public static PushPayload buildPushObjectIosListaliasAlert(List<String> alias, String alert, boolean isSound) {
        SendMessage message = GsonUtil.gson2Object(alert, SendMessage.class);
        IosAlert iosAlert = IosAlert.newBuilder()
                .setTitleAndBody(message.getSendTitle(), "", message.getSendContent())
                .build();
     ...
     *              Date: 2017/8/2 16:27
     */
    public static void pushList(List<String> alias, String alert,boolean isSound) {
        PushPayload payload = buildPushObjectAndroidIosListaliasAlert(alias, alert);
        PushPayload payloadIos = buildPushObjectIosListaliasAlert(alias, alert, isSound);
        try {
            jpushClient.sendPush(payloadIos);
        } catch (Exception e) {
