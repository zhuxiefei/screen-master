package com.betel.estatemgmt.utils.pagination;
...
import com.betel.estatemgmt.utils.pagination.helper.DialectHelper;
import com.betel.estatemgmt.utils.pagination.helper.SqlHelper;
...
public class PaginationInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (intercept && dialect.supportsLimit() && (offset != RowBounds.NO_ROW_OFFSET || limit != RowBounds.NO_ROW_LIMIT)) {
            ...
            //由总记录数求得新的offset
            int isZero=count%limit;
            int totalPage=count/limit;
            if (offset>=count){
                if (isZero==0){
                    offset=(totalPage-1)*limit;
                    if ( logger.isDebugEnabled()){
                        logger.debug("===分页插件===count%limit==>>整除=========="+offset);
                    }
                    if (offset<0){
                        offset=0;
                    }
                }else if (isZero!=0){
                    offset=(totalPage+1-1)*limit;
                    if ( logger.isDebugEnabled()){
                        logger.info("===分页插件==count%limit==>>>>不整除=========="+offset);
                    }
                }
            }
    ...
    public static class RawSqlSource implements SqlSource {
