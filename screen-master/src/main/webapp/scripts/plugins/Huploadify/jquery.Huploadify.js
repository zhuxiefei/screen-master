(function($){
	$.fn.Huploadify = function(opts){
		var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename" title="${fileName}">${fileName}</span><a href="javascript:void(0)" class=" uploadbtn"  >上传</a><a href="javascript:void(0)"  class=" delfilebtn"  >删除</a></div>';
		var defaults = {
			...
			fileNameLength:120,//允许上传的文件的名称长度
		...
		this.each(function(index, element){
			...
			var uploadManager = {
				...
				init : function(){
					var inputStr = '<label class="row-label" style=" margin-top: 4px;">上传文件：</label><input id="select_btn_'+instanceNumber+'" class="selectbtn" style="display:none;" type="file" name="fileselect[]"';
					inputStr += option.multi ? ' multiple' : '';
					inputStr += ' accept="';
					inputStr += F.getFileTypes(option.fileTypeExts).join(",");
					inputStr += '"/>';
					inputStr += '<a id="file_upload_'+instanceNumber+'-button" href="javascript:void(0)" class="uploadify-button"  style=" margin-top: 0;">';
					...
					if(typeArray.length>0){
						for(var i=0,len=files.length;i<len;i++){
							...
							if($.inArray('*',typeArray)>=0 || $.inArray(f.name.split('.').pop().toLowerCase(),typeArray) < 0){
								BJUI.alertmsg('error','请上传'+typeArray.toString()+'格式的文件！');
								$.CurrentDialog.find('#select_btn_'+instanceNumber).val("");
								return
							}
							if(parseFloat(F.formatFileSize(f.size,true))<=option.fileSizeLimit&&parseFloat(F.formatFileSize(f.size,true))>0){
								if($.inArray('*',typeArray)>=0 || $.inArray(f.name.split('.').pop().toLowerCase(),typeArray)>=0){

									if(f.name.length>option.fileNameLength){
										BJUI.alertmsg('error','文件名称不能超过'+option.fileNameLength+'个字！');
										$.CurrentDialog.find('#select_btn_'+instanceNumber).val("");
									}else{
										arr.push(f);
									}
									...
									//BJUI.alertmsg('error','文件 "'+f.name+'" 类型不允许！');
									BJUI.alertmsg('error','请上传'+typeArray.toString()+'格式的文件！');
									$.CurrentDialog.find('#select_btn_'+instanceNumber).val("");
								}
							}
							else if(parseFloat(F.formatFileSize(f.size,true)) == 0){
								BJUI.alertmsg('error','文件不能为空');
								$.CurrentDialog.find('#select_btn_'+instanceNumber).val("");
								continue;
							}
							else{
								//alert('文件 "'+f.name+'" 大小超出限制！');
								//BJUI.alertmsg('error','文件 "'+f.name+'" 大小超出限制的5M！');
								BJUI.alertmsg('error','文件大小不能超过'+(option.fileSizeLimit/1024)+'M！');
								$.CurrentDialog.find('#select_btn_'+instanceNumber).val("");
								continue;
							}
						}
				...
				_renderFile : function(file){
					...
					if(option.showUploadedSize){
						var num = '<span class="progressnum"><span class="uploadedsize">0KB</span><span>/</span><span class="totalsize">${fileSize}</span></span>'.replace(/\${fileSize}/g,F.formatFileSize(file.size));
				...
				_getFiles : function(e){
					...
					if(_this.find('.uploadify-queue .uploadify-queue-item').length>0){  //判断队列是否为空
						var item = _this.find('.uploadify-queue .uploadify-queue-item:last').attr("id").lastIndexOf("_");   //最后一个任务的id开始累加
						var fileCount = _this.find('.uploadify-queue .uploadify-queue-item:last').attr("id").substring(item+1);
					}else{
						var fileCount = _this.find('.uploadify-queue .uploadify-queue-item').length; //队列中已经有的文件个数    从0开始累加
					}
		  			for(var i=0,len=files.length;i<len;i++){
					...
					try{
				  	...
				  	if(xhr.upload){
						...
						if(file.status===0){
							...
							xhr.setRequestHeader('estateId',$.cookie("estateId"));
							var fd = new FormData();
