...
+function ($) {
    ...
    var BJUI = {
        ...
        eventType: {
            ...
            //custom
            loadSideNav  : 'bjui.custome.initSideNav'
        },
        formColWidth: {L:900, M:680, S:360, SS:240},
        pageInfo: {curPage:'curPage', pageSize:'pageSize', orderField:'orderField', orderDirection:'orderDirection'},
        alertMsg: {displayPosition:'middlecenter', alertTimeout: 6000}, //alertmsg display position && close timeout
        ajaxTimeout: 30000,
        //TODO 此处的error和timeout要根据代码逻辑重置制定
        statusCode: {ok:'00000', error:'99999', timeout:'xxxxx'},
        keys: {statusCode:'code', message:'message'},
        ui: {
            windowWidth      : 0,
            showSideWidth    : 990,
            sidenavWidth     : 260,
    ...
    $(function() {
        ...
        $.validator && $.validator.config({
            ...
            rules: {
                ...
                ,chinese: [/^[\u0391-\uFFE5]+$/, '请输入汉字']
                ,username: [/^[\u4e00-\u9fa5\w-_]{2,12}$/, '请输入2-12位中英文、数字、-、_']
                ,uName: [/^[a-zA-Z\u4e00-\u9fa5\u0020]*$/, '请输入中英文']
                ,usernickname: [/^[\u4e00-\u9fa5\w-_]{2,15}$/, '请输入2-15位中英文、数字、-、_']
                //,adminAccount: [/^[-_a-zA-Z\u4e00-\u9fa5][\w-_\u4e00-\u9fa5]{5,19}$/, '请输入6-20位中英文、数字、-、_ ,且不能以数字开头']
                ,adminAccount: [/^(?![0-9])[\w-_\u4e00-\u9fa5]{6,20}$/, '请输入6-20位中英文、数字、-、_ ,且不能以数字开头']
                ,adminname: [/^[\w-_\u4e00-\u9fa5]{2,15}$/, '请输入2-15位中英文、数字、-、_']
                //,password: [/^[a-zA-Z0-9,./!@#$%^&\S]{6,20}$/, "请填写6-20位中英文、数字、,./!@#$%^&"]
                //,password: [/^(?=.*[0-9].*)(?=.*[A-Z].*)(?=.*[a-z].*).([A-Z]|[a-z]|[0-9]|[,./!@#$%^&]){5,19}$/, "请填写6-20位的大写字母、小写字母和数字的组合，不能使用特殊字符（可使用字符,./!@#$%^&）"]
                ,password: [/^(?=.*[0-9].*)(?=.*[A-Z].*)(?=.*[a-z].*).([A-Z]|[a-z]|[0-9]|[,./!@#$%^&]){5,19}$/, "密码格式有误，请填写正确的密码"]
                ,searchKeyword: [/^[^'"<>%\\]+$/, "请填写0-30位的字，不能含有\'\"<>%\\"]
                ,searchillegal: [/^[^'"<>%\\]+$/, "请填写1-30位的字，不能含有\'\"<>%\\"]
                ,searchKeywords: [/^[^'"<>%\\]+$/, "请填写0-120位的字，不能含有\'\"<>%\\"]
                ,titleVerify: [/^[^'"<>%\\]+$/, "请填写1-20位的字，不能含有\'\"<>%\\"]
                ,searchName: [/^[^'"<>%\\]+$/, "请填写0-20位的字，不能含有\'\"<>%\\"]
                ,searchPublisher: [/^[^'"<>%\\]+$/, "请填写0-15位的字，不能含有\'\"<>%\\"]
                ,searchHouseInfo: [/^[^'"<>%\\]+$/, "请填写0-50位的字，不能含有\'\"<>%\\"]
                ,titleVerify4: [/^[^'"<>%\\,]+$/, "请填写1-10位的字，不能含有\'\"\,<>%\\"]
                ,titleVerify2: [/^(?![^'"<>%\\]+$){0,20}$/, "请填写0-20位的字，不能含有\'\"<>%\\"]
                ,titleVerify3: [/^[^'"<>%,\\]+$/, "请填写1-20位的字，不能含有\'\"<>%\\\,"]
                ,special: [/^[^'"<>%\\]+$/, "请重新填写，不能含有\'\"<>%\\"]
                // ,houseArea: [/^(^0\.\d{0,1}[1-9]$)|(^[1-9]\d{0,4}\.\d{0,1}[1-9]$)|(^[1-9]\d{0,4}$)$/, "单位平米，整数部分不超过5位，小数部分不超过2位（例如：12345.00）"]
                ,houseArea: [/^(^0\.\d{1,2}$)|(^\d{1,5}\.\d{1,2}$)|(^\d{1,5}$)$/, "整数部分不超过5位，小数部分不超过2位"]
                ,numberOnly: [/^(^[0-9]+([.]{1}[0-9]+){0,1}$)$/, "请填写整数或者小数"]
                ,numberOnly1: [/^[0-9]+$/, "请输入数字"]
                ,notZero: [/^((?!0+$) )$/, "面积不能为0"]
                ,notZero1: [/([1-9]\d*(\.\d*[1-9])?)|(0\.\d*[1-9])/, "价格不能为0"]
                ,notZero2: [/([1-9]\d*(\.\d*[1-9])?)|(0\.\d*[1-9])/, "面积不能为0"]
                // ,numberOnly: [/^((^0\.\d{1,2}$)|(^\d{1,5}\.\d{1,2}$)|(^[1-9]|\d{2,5}$))$/, "请填写正整数或者小数"]
                ,phoneNumber: [/^(^1[3|4|5|7|8][0-9]\d{8}$)$/, "请输入正确的联系电话"]
                ,itemPrice: [/^((([1-9]\d{0,3}|0|[1-4]\d{4})(\.\d{1,2})?)|50000|50000.0|50000.00)$/, "请填写大于0小于等于50000的<br/>数,且小数点后最多只能保留两位"]
                ,itemPriceBig: [/^((([1-9]\d{0,5})(\.\d{1,2})?)|1000000|1000000.0|1000000.00)$/, "请填写大于等于1小于等于1000000的数，<br/>且小数点后最多只能保留两位"]
                ,overdueFine: [/^((?!^0\.0?0$)^[0-9][0-9]?(\.[0-9]{1,2})?$|^100$)$/, "请填写大于0小于100的数，<br/>且小数点后只能保留两位"]
                ,chargeCycle: [/^[1-9][0-9]$/, "请填写大于9小于100的整数"]
                ,itemCycle: [/^(([1-9])|(1[0-9])|(2[0-4]))$/, "请填写大于0小于24的整数"]
                ,englishNum: [/^[0-9a-zA-Z]+$/, "请输入英文或者数字"]
                ,number10000: [/^(([1-9]\d{0,3})|10000)$/, "请输入1-10000的整数"]
                ,number1000: [/^(([1-9]\d{0,2})|1000)$/, "请输入1-1000的整数"]
                ,chineseEnglishNum: [/^[A-Za-z0-9\u4e00-\u9fa5]+$/, "请输入中文，英文，数字"]
                ,fullTime: [/^[^-]+$/, "请填写正确的时间"]
                ,dotNum: [/^(?!0+(?:\.0+)?$)\d+(?:\.\d{1,2})?$/, "小数部分不能超过2位"]
                ,phoneNum: [/^[\d()\-\s]*$/, "只能出现数字，中划线，空格，括号字符"]
                ...
                ,overdueFine1:function(element, params) {
                    // if (!params) return true
                    ...
                    return (boolean ? this.renderMsg('请填写大于0小于100的数，且小数点后只能保留两位', params) : true)
                }
...
+function ($) {
    ...
    window.FRAG = {
        ...
        '    </li>' +
        '    <li class="page-jumpto"><span class="page-input"><input class="form-control input-sm-pages" type="text" size="3.2" value="#curPage#" title="#jumpto#"></span></li>' +
...
+function ($) {
    ...
    $.fn.extend({
        ...
        doAjax: function(op) {
            ...
            if(!op.enctype){   //是否是formData
                if (!op.dataType) op.dataType = 'json'
            }
            if (!op.cache) op.cache = false
            op.timeout = op.ajaxTimeout || BJUI.ajaxTimeout
            //if (!op.contentType) op.contentType = 'application/json;charset=utf-8'
            if(op.contentType == undefined || new String(op.contentType).valueOf().trim() == ""){
                op.contentType = 'application/json;charset=utf-8'
            }
            var baseHeader = {"userId":$.cookie("l_userId"),"token":$.cookie("l_token"),"estateId":$.cookie("estateId")}
            op.headers = !op.headers ? baseHeader : $.extend(op.headers, baseHeader)
            ...
            if(!op.enctype){   //是否是formData
                if (op.data && typeof op.data != 'string')
                {
                    ...
                    for(var key in op.data){
                        if(op.data[key]!=null){
                            cup[key]=$.trim(op.data[key]);
                        }
                    }
                    op.data = JSON.stringify(cup)
                }
            }
            $.ajax(op)
        },
        ...
        serializeJson: function () {
            ...
            $.each(a, function () {
                var searchdata = this.value.trim();
                if (o[this.name] !== undefined) {
                    if (!o[this.name].push) {
                        o[this.name] = [o[this.name]]
                    }
                    o[this.name].push(searchdata || '')
                } else {
                    o[this.name] = searchdata || ''
                }
            })
...
+function ($) {
    ...
    Sidenav.prototype.ajaxHnav = function() {
        ...
        var createA = function(data) {
            ...
            liHtml.add('<a href="'+ (data.url+"?v="+BJUI_HTML_VERSIONS || 'javascript:;') +'" ')
            ...
            if (data[childKey]){
                ...
                liHtml.add('><i class=""></i>&nbsp;'+ data.name)
            }
        ...
        var treeCallback = function(json) {
         ...
         menuForm = [
            {"name":"小区管理","menuId":"1","menuName":"community", "children":[
                {"id":"documentManage", "name":"资料管理","menuName":"documentManage", "target":"navtab","fresh":true,
                    "url":"html/community/documentManage.html"},
                {"id":"notice", "name":"公告维护","menuName":"notice", "target":"navtab","fresh":true,
                    "url":"html/community/notice.html"},
                {"id":"activity", "name":"小区工作日志","menuName":"activity", "target":"navtab","fresh":true, "url":"html/community/activity.html"}

            ]},
            {"name":"缴费管理","menuId":"5","menuName":"expenseManage", "children":[
                {"id":"paymentManage", "name":"费用管理","menuName":"paymentManage", "target":"navtab","fresh":true,
                    "url":"html/property/paymentManage.html"},
                {"id":"income", "name":"收入明细","menuName":"income", "target":"navtab","fresh":true,
                    "url":"html/property/income.html"},
                {"id":"itemSetting", "name":"收费项目设置","menuName":"itemSetting", "target":"navtab","fresh":true,
                    "url":"html/property/itemSetting.html"},
                {"id":"paymentRemind", "name":"费用提醒","menuName":"paymentRemind", "target":"navtab","fresh":true,
                    "url":"html/property/paymentRemind.html"}
            ]},
             {"name":"维修管理","menuId":"2","menuName":"repairManage", "target":"navtab","fresh":true,
                 "url":"html/repairManage/repairManage.html"},
             {"name":"设施巡检记录","menuId":"11","menuName":"facilitiesPatrol", "target":"navtab","fresh":true,
                 "url":"html/patrolContent/patrolContent.html"},
             {"name":"房屋管理","menuId":"12","menuName":"house", "children":[
                 {"id":"buildingManage", "name":"楼宇信息维护","menuName":"buildingManage", "target":"navtab","fresh":true,
                     "url":"html/house/buildingList.html"},
                 {"id":"houseTypeManage", "name":"户型维护","menuName":"houseTypeManage", "target":"navtab","fresh":true,
                     "url":"html/house/houseTypeList.html"},
                 {"id":"materialManage", "name":"家装建材维护","menuName":"materialManage", "target":"navtab","fresh":true, "url":"html/house/fitmentList.html"},
                 {"id":"houseManage", "name":"房屋维护","menuName":"houseManage", "target":"navtab","fresh":true, "url":"html/house/houseList.html"},
                 //(写字楼)
                 {"id":"officeHouseManage", "name":"房屋维护","menuName":"officeHouseManage", "target":"navtab","fresh":true, "url":"html/house/houseList_office.html"},
                 {"id":"decoration", "name":"装修维护","menuName":"doDecoration", "target":"navtab","fresh":true,
                     "url":"html/house/decoration.html"}
                 /*{"id":"houseStatus", "name":"房屋状态","menuName":"houseStatus", "target":"navtab","fresh":true, "url":"html/house/houseStatus.html"},
                  {"id":"authentication", "name":"户主认证","menuName":"authentication", "target":"navtab","fresh":true,
                  "url":"html/todo/authentication.html"},*/
             ]},
             {"name":"任务管理","menuId":"18","menuName":"taskManage", "children":[
                 {"id":"taskType", "name":"任务类型","menuName":"taskType", "target":"navtab","fresh":true,
                     "url":"html/task/taskType.html"},
                 {"id":"task", "name":"任务内容","menuName":"task", "target":"navtab","fresh":true,
                     "url":"html/task/taskContent.html"},
             ]},
             {"name":"任务统计","menuId":"19","menuName":"taskCount","target":"navtab","fresh":true, "url":"html/task/taskStatistics.html"},
             {"name":"设备管理","menuId":"21","menuName":"equipmentManage", "children":[
                 {"id":"equipmentType", "name":"设备类型","menuName":"equipmentType", "target":"navtab","fresh":true,
                     "url":"html/equipment/equipmentType.html"},
                 {"id":"equipmentList", "name":"设备列表","menuName":"equipmentList", "target":"navtab","fresh":true,
                     "url":"html/equipment/equipmentList.html"},
                 {"id":"maintenance", "name":"维保记录","menuName":"maintenance", "target":"navtab","fresh":true,
                     "url":"html/equipment/repairRecord.html"},
             ]},
             {"name":"保洁管理","menuId":"22","menuName":"cleaning", "children":[
                 {"id":"cleaningArea", "name":"保洁区域与类型","menuName":"cleaningArea", "target":"navtab","fresh":true,
                     "url":"html/cleaning/cleaningArea.html"},
                 {"id":"cleaningContent", "name":"保洁内容","menuName":"cleaningContent", "target":"navtab","fresh":true,
                     "url":"html/cleaning/cleaningContent.html"},
                 {"id":"cleaningRecord", "name":"保洁考核记录","menuName":"cleaningRecord", "target":"navtab","fresh":true,
                     "url":"html/cleaning/cleaningRecord.html"},
             ]},
             {"name":"安保巡逻","menuId":"23","menuName":"security", "children":[
                 {"id":"securityArea", "name":"巡逻区域","menuName":"securityArea", "target":"navtab","fresh":true,
                     "url":"html/security/securityArea.html"},
                 {"id":"securityContent", "name":"巡逻内容","menuName":"securityContent", "target":"navtab","fresh":true,
                     "url":"html/security/securityContent.html"},
                 {"id":"securityRecord", "name":"巡逻记录","menuName":"securityRecord", "target":"navtab","fresh":true,
                     "url":"html/security/securityRecord.html"},
             ]},
             {"name":"租赁管理","menuId":"2","menuName":"lease", "target":"navtab","fresh":true,
                 "url":"html/rentManage/rent.html"},
             {"name":"系统设置","menuId":"25","menuName":"system", "children":[
                 {"id":"suggest", "name":"意见反馈","menuName":"suggest", "target":"navtab","fresh":true,
                     "url":"html/system/suggest.html"},
             ]},
        ...
        //handle data of menus, convert to map  提取需要匹配的返回值
        for (var index = 0; index < BJUI.MENUlLIST.length; index++)
        {
            menuObj[BJUI.MENUlLIST[index].menuName] = BJUI.MENUlLIST[index];
        }

        //遍历一级菜单
        for (var i = 0; i < menuForm.length; i++)
        {
            if (menuObj[menuForm[i].menuName])
            {
                newMenuForm.push(menuForm[i]);
            }
        }
        for(var a =0; a<newMenuForm.length; a++){

            if(newMenuForm[a].children && newMenuForm[a].children.length > 0){

            var secondNewMenuList = newMenuForm[a].children;
            var secondMenuList = [];
            for(var b=0; b<secondNewMenuList.length; b++){
                console.log(secondNewMenuList.length);
                if(menuObj[secondNewMenuList[b].menuName]){
                    //secondNewMenuList.remove(secondNewMenuList[b].menuName);
                    secondMenuList.push(secondNewMenuList[b]);
                }
            }
            newMenuForm[a].children = secondMenuList;

            }
        }
        console.log(newMenuForm)
        okCallback(newMenuForm);
        //okCallback(BJUI.MENUlLIST);
    ...
    Plugin.call($('[data-toggle="sidenav"]'), 'initHnav');
    $(document).on(BJUI.eventType.loadSideNav, '[data-toggle="sidenav"]', function(e) {
        // e.preventDefault()
...
+function ($) {
            ...
            bindings: {
                reload: function(t, m) {
                    if (t.data('options').url)
                        that.refresh(t.data('options').id,1)
                },
    ...
    Navtab.prototype.refresh = function(tabid,cup) {
        var $tab, $panel
        var selectNavtabIndex=this.tools.indexTabId(tabid)
        if(selectNavtabIndex>=0){
            var selectNavtabName=this.tools.getTabs()[selectNavtabIndex].textContent.substr(0,this.tools.getTabs()[selectNavtabIndex].textContent.length-1);
            var currentNavtabName=$currentTab[0].textContent.substr(0,$currentTab[0].textContent.length-1);
            if (!tabid) {
                $tab = $currentTab
            } else if (selectNavtabName!=currentNavtabName&&cup) {
                $tab = $currentTab
...
+function ($) {
            ...
            reload: function($dialog, options) {
                ...
                if (options.url) {
                    ...
                    $dialogContent.ajaxUrl({
                        type:options.type || 'GET', url:options.url+'?v='+BJUI_HTML_VERSIONS, data:data || {}, loadingmask:options.loadingmask, callback:function(response) {
                    ...
                    if (options.image) {
                        html = '<div style=" text-align: center;"><img src="'+ decodeURIComponent(options.image) +'" style="max-width:100%;"></div>'
    ...
    Dialog.prototype.close = function(dialog) {
        ...
        if (options.noanimate) {
            ...
            $dialog.trigger(BJUI.eventType.beforeCloseDialog).remove();    //解决关闭弹窗里面下拉的延迟
    ...
    Bjuiajax.prototype.ajaxcallback = function(json) {
        ...
        var okFunc = function() {
                ...
                if (typeof okCallback === 'function')
                    okCallback.apply(that, [json.data || json, options])
                else
        ...
        if (json[BJUI.keys.statusCode] == BJUI.statusCode.ok) {
            ...
            //successBack(json[BJUI.keys.statusCode]);
            successBack(json);
            /*console.log(json[BJUI.keys.statusCode]);
            console.log(json[BJUI.keys]);*/
            if (errCallback) {
                errFunc()
            }
        }
    ...
    Bjuiajax.prototype._ajaxform = function($form, options) {
        ...
        var _submitFn = function() {
            ...
            if (enctype && enctype == 'multipart/form-data') {
                if (window.FormData) {
                    $.extend(op, {data:new FormData($form[0]), contentType:false, processData:false,enctype:true})

                } else {
                    $.extend(op, {data:$form.serializeArray(), files:$form.find(':file'), iframe:true, processData:false})
                }
            } else {
                $.extend(op, {data:$form.serializeJson()})
            }
...
+function ($) {
    ...
    Alertmsg.DEFAULTS = {
        displayPosition : 'middlecenter', // Optional 'topleft, topcenter, topright, middleleft, middlecenter, middleright, bottomleft, bottomcenter, bottomright'
...
+function ($) {
    ...
    Pagination.prototype.TOOLS = function() {
        ...
        var tools   = {
            ...
            getPageInterval: function(count, curPage, showPageNum) {
                var half  = Math.ceil(showPageNum / 2), limit = count - showPageNum,
                    start = curPage > half ? Math.max(Math.min(curPage - half, limit), 0) : 0,
                    end   = curPage > half ? Math.min((curPage + half), count) : Math.min(showPageNum, count)
            ...
            // jump to page
            jumpPage: function(curPage, pageSize) {
                ...
                if (curPage) {
                    that.options.curPage = curPage
                }
                if (pageSize) {
                    that.options.pageSize = pageSize
                    that.pageCount = this.getPageCount(pageSize, that.options.total)

                    if (that.options.curPage > that.pageCount)
                        that.options.curPage = that.pageCount
                }

                if (callback && typeof callback === 'function') {
                    callback.call(that, that.options.curPage, pageSize)
                } else {
                    this.setPaging(that.options.curPage, pageSize, true)
                }

                that.$element.trigger('bjui.pagination.jump')
            },
            setPaging: function(curPage, pageSize, isSubmit) {
                ...
                if ($form) {
                    ...
                    paging[BJUI.pageInfo.curPage] = curPage
    ...
    Pagination.prototype.jumpPage = function(curPage, pageSize) {
        var that = this, options = that.options, pageCount = that.pageCount

        if (curPage && isNaN(curPage)) return
        ...
        if (curPage) {
            ...
            if (curPage == options.curPage) return
        }
        if (pageSize) {
            pageSize = parseInt(pageSize, 10)
        }

        that.tools.jumpPage(curPage || options.curPage, pageSize || options.pageSize)
    }
    ...
    Pagination.prototype.init = function() {
        ...
        interval = tools.getPageInterval(that.pageCount, options.curPage, options.showPagenum)

        for (var i = interval.start; i < interval.end; i++) {
            pageNums.push(FRAG.gridPageNum.replace('#num#', i).replace('#active#', (options.curPage == i ? ' active' : '')))
        }

        btnpaging = BJUI.doRegional(btnpaging.replaceAll('#curPage#', options.curPage).replaceAll('#count#', options.total +'/'+ parseInt((that.pageCount || 0), 10)), pr)
        ...
        tools.setPaging(options.curPage, options.pageSize)
        ...
        if (options.curPage == 1) pageFirst()
        if (options.curPage == that.pageCount) {
            pageLast()
            if (options.curPage == 1) disablePrev()
        }
        ...
        that.$element.on('click.bjui.pagination.pagenum', 'li.page-num', function(e) {
            ...
            var curPage = that.options.curPage, interval = tools.getPageInterval(that.pageCount, curPage, options.showPagenum), pageNums = []

            for (var i = interval.start; i < interval.end; i++) {
                pageNums.push(FRAG.gridPageNum.replace('#num#', i).replace('#active#', (curPage == i ? ' active' : '')))
            }
            ...
            if (curPage == 1) {
                pageFirst()
                if (curPage == that.pageCount) disableNext()
                if (!that.options.total) disableNext()
            } else if (curPage == that.pageCount) {
                pageLast()
            } else {
                enablePrev()
                enableNext()
            ...
            $jumpto.find('input').val(curPage)
                var totalVal = $('body').attr('currentPageTotal');
                if (totalVal) {
                    $pagetotal.find('> span').html(totalVal+ '/' + that.pageCount)
                } else {
        ...
        $prev.on('click', function() {
            if (that.options.curPage > 1)
                that.jumpPage(that.options.curPage - 1)
        })
...
+function ($) {
    ...
    $(document).on('click.bjui.datepicker.data-api', '[data-toggle="datepickerbtn"]', function(e) {
       var noDate= $(this).parent().html();
        var aa=parseDom(noDate);
        if(aa[0]['attributes']['data-month']||aa[1]['attributes']['data-month']){
            $('#bjui-calendar .body').hide();
        }else{
            $('#bjui-calendar .body').show();
        }
...
+function ($) {
        ...
        var tools = {
            ...
            createTrsByData: function(data, refreshFlag) {
                ...
                if (data) {
                    ...
                    that.paging.total = data.total || 0
                    ...
                    if (that.paging.curPage > that.paging.pageCount) that.paging.curPage = that.paging.pageCount
                    if (!that.paging.curPage) that.paging.curPage = 1
                ...
                var doInit = function() {
                    ...
                    if (!allData.length) {
                        ...
                        if (options.local === 'local') {
                            start = (paging.pageSize * (paging.curPage - 1))
            ...
            createTrs: function(datas, refreshFlag) {
                ...
                if(that.checkboxColumn.th !== undefined){
                    that.checkboxColumn.th.iCheck("uncheck");   //清空全选框
                }
                ...
                for (var i = 0, l = datas.length; i < l; i++) {
                    var trData = datas[i], modellength = model.length, tempcolspan = modellength, linenumber = options.linenumberAll ? ((paging.curPage - 1) * paging.pageSize + (i + 1)) : (i + 1),
                        ...
                        if (that.isTemplate) {
                            ...
                            if (!tds[0] && j == (modellength - 1)) {
                                ...
                                if (that.options.showLinenumber) {
                                    ...
                                    var linenumbertd = ['<td data-title="No." align="center" class="datagrid-linenumber-td"'+ display +'>'+ 1 +'</td>']
                            ...
                            if (tree) {
                            	...
                            	var _displayValue = typeof render_label === 'undefined' ? label : render_label
                                if (n.nowrap && !/.*<[a-zA-Z]+>.*/.test(_displayValue))
                                {
                                    tdHtml.add("<div style='word-wrap: normal; white-space:nowrap;overflow:hidden;text-overflow:ellipsis;' title='"+_displayValue+"'>")
                                }
                                else
                                {
                                    tdHtml
            ...
            loadData: function(data, refreshFlag) {
                ...
                if (!data.pageSize && options.paging) {
                    ...
                    data.curPage = that.paging.curPage
                }
                ...
                // if(options.data&&options.preShow){
                //     tools.createTrsByData(options.data, refreshFlag)
                // }else{
                if(options.preShow){
                    options.paging=true;
                }
                    BJUI.ajax('doajax', {
                        ...
                        errCallback: function(json) {
                            ...
                            if (typeof options.dataErrorCallBack === 'function') {
                                options.dataErrorCallBack(json);
                            }
                            //console.log(json);
                            successBack(json);
            ...
            jumpPage: function(curPage, pageSize) {
                ...
                if (curPage) {
                    that.paging.oldcurPage = that.paging.curPage
                    that.paging.curPage    = curPage
                }
                if (pageSize) {
                    ...
                    if (that.paging.curPage > that.paging.pageCount)
                        that.paging.curPage = that.paging.pageCount
                }
            ...
            quickSort: function(model) {
                ...
                if (options.sortAll) {
                    if (options.local === 'remote') {
                        ...
                        postData[BJUI.pageInfo.curPage] = that.paging.curPage
            ...
            getRemoteFilterData: function(isPaging, msgs) {
                ...
                if (that.filterDatas && !$.isEmptyObject(that.filterDatas)) {
                    ...
                    if (!isPaging) that.paging.curPage = 1
                }
                ...
                filterDatas[BJUI.pageInfo.curPage] = that.paging.curPage || 1
            ...
            updateGridIndex: function() {
                var paging = that.paging, options = that.options, startNumber = (paging.curPage - 1) * paging.pageSize
    ...
    Datagrid.prototype.init = function() {
        ...
        that.paging = $.extend({}, {pageSize:30, selectPageSize:'30,60,90', curPage:1, total:0, showPagenum:5}, (typeof options.paging === 'object') && options.paging)
    ...
    Datagrid.prototype.refresh = function(filterFlag) {
        ...
        if (!options.postData || !options.postData[pageInfo.pageSize]) {
            ...
            postData[pageInfo.curPage] = paging.curPage
        }
    ...
    Datagrid.prototype.initEvents = function($trs) {
        ...
        that.$grid.off('click.bjui.datagrid.tr.child').on('click.bjui.datagrid.tr.child', 'td.'+ that.classnames.td_child, function(e) {
            ...
            // th checkbox - check all  //ifChanged
            .off('ifChanged').on('ifClicked', 'th.'+ that.classnames.td_checkbox +' input', function(e) {
            e.stopPropagation()
            ...
            $trs
                ...
                if (afterDrop && that.afterDrop) {
                    if (typeof afterDrop === 'function') {
                        ...
                        var postData = {pageSize:that.paging.pageSize, curPage:that.paging.curPage}, type = options.editType, opts = {url:options.dropOptions.dropUrl, type:'POST', reload:false}
            ...
            that.$tbody.find('> tr').each(function(i) {
                ...
                if ($tr.hasClass(that.classnames.tr_child) || $tr.hasClass('datagrid-nodata'))
                    ...
                    if (options.linenumberAll)
                        linenumber = ((paging.curPage - 1) * paging.pageSize + (linenumber))
    ...
    Datagrid.prototype.initPaging = function() {
        ...
        interval = tools.getPageInterval(pageCount, paging.curPage, paging.showPagenum)

        for (var i = interval.start; i < interval.end; i++) {
            pageNums.push(FRAG.gridPageNum.replace('#num#', i).replace('#active#', (paging.curPage == i ? ' active' : '')))
        }

        btnpaging = BJUI.doRegional(btnpaging.replaceAll('#curPage#', paging.curPage).replaceAll('#count#', paging.total +'/'+ parseInt((pageCount || 0), 10)), pr)
        ...
        if (paging.curPage == 1) pageFirst()
        if (paging.curPage == paging.pageCount) {
            pageLast()
            if (paging.curPage == 1) disablePrev()
        }
        ...
        that.$boxP.on('click.datagrid.pagenum', 'li.page-num', function(e) {
            ...
            var curPage = that.paging.curPage, interval = tools.getPageInterval(that.paging.pageCount, curPage, paging.showPagenum), pageNums = []
            ...
            if (curPage == 1) {
                pageFirst()
                if (curPage == that.paging.pageCount) disableNext()
                if (!that.paging.total) disableNext()
            } else if (curPage == that.paging.pageCount) {
                pageLast()
            } else {
                enablePrev()
                enableNext()
        ...
        $first.on('click', function() {
            if (that.paging.curPage > 1)
                that.jumpPage(1)
        })

        $prev.on('click', function() {
            if (that.paging.curPage > 1)
                that.jumpPage(that.paging.curPage - 1)
        })
    ...
    Datagrid.prototype.jumpPage = function(curPage, pageSize) {
        ...
        if (curPage) {
            ...
            if (curPage < 1)         curPage = 1
            if (curPage > pageCount) curPage = pageCount
            if (curPage == paging.curPage) return
        }
        if (pageSize) {
            pageSize = parseInt(pageSize, 10)
        ...
        that.tools.jumpPage(curPage, pageSize)
    }
    ...
    Datagrid.prototype.add = function(addLocation, addData, noEdit) {
        ...
        var that = this, options = that.options, keys = options.keys, tools = that.tools, paging = that.paging, trs, obj = {}, data = [], addObj, startNumber = (paging.curPage - 1) * paging.pageSize, linenumber
        ...
        if (that.allData.length) {
            ...
            if(that.allData instanceof Array){
                that.allData.push(addData);
            }else{
                that.allData = that.allData.rows;
                that.allData.push(addData);
            }
    ...
    Datagrid.prototype.delRows = function(rows) {
        ...
        var doDel = function() {
            ...
            // remote delete
            if (options.delUrl) {
                var postData = [], callback = options.delCallback,errorCallback = options.delErroeCallback

                if(options.customData){ // 自定义data
                    var type = options.delType, opts = {url:options.delUrl, data:options.customData(), type:'POST', okCallback:callback || function(json) { delEnd() },errCallback:errorCallback}
                }else{
                    ...
                    var createDelData = function(){
                        var data = {};
                        data[options.delPKKey] = postData.join(',')
                        return data
                    }
                    var type = options.delType, opts = {url:options.delUrl, data:(options.delPK ? JSON.stringify(createDelData()) : JSON.stringify(postData)), type:'POST', okCallback:callback || function(json) { delEnd() },errCallback:errorCallback}
...
+function ($) {
    ...
    $(document).one(BJUI.eventType.afterInitUI, function(e) {
        ...
        $.ajax({
            type: "POST",
            url: path+'/web/admin/v1/findAdminInfo',
            contentType:'application/json;charset=UTF-8',
            dataType:'json',
            headers:{"userId":$.cookie("l_userId"),"token":$.cookie("l_token"),"estateId":$.cookie("estateId")},
            success: function(json) {
                if(json.code==BJUI.statusCode.ok){
                    $("#dataUserName").text(json.data.name);
                    $("body").data("dataUserName",json.data.name);
                    BJUI.MENUlLIST=json.data.menuList;
                    $a.trigger(BJUI.eventType.loadSideNav);
                    var cup=[];
                    for(var i=0;i<json.data.menuList.length;i++){
                        if(json.data.menuList[i].menuType==4){
                            cup.push(json.data.menuList[i].menuName)
                        }
                    }
                    var oaPort=8099;
                    var socialPort=8098;
                    ...
                    var pathPortView=window.location.port;
                    var pathIndexView=pathAllView.indexOf(pathPortView);
                    var pathExView=pathAllView.slice(0,pathIndexView);
                    if(cup.length==1&&cup.indexOf('estatemgmt')>=0){
                        var html='<li><a href="#" style="color:#999;font-size:11px">&nbsp;<i>暂无其它平台权限</i></a></li>'
                        $('ul#platformUl').append(html);
                    }
                    if(cup.indexOf('social')>=0){
                        var url=pathExView+socialPort+'/index.html?reLogin';
                        var html='<li><a href="'+url+'" style="font-weight:bold;">&nbsp;社交管理系统平台</a></li>'
                        $('ul#platformUl').append(html);
                    }
                    // if(cup.indexOf('estatemgmt')>=0){
                    //     var url=pathExView+estatePort+'/index.html';
                    //     var html='<li><a href="'+url+'" style="font-weight:bold;">&nbsp;物业管理系统平台</a></li>'
                    //     $('ul#platformUl').append(html);
                    // }
                    if(cup.indexOf('oa')>=0){
                    var url=pathExView+oaPort+'/index.html?reLogin';
                        var html='<li><a href="'+url+'" style="font-weight:bold;">&nbsp;OA管理系统平台</a></li>'
                        $('ul#platformUl').append(html);
                    }
                    if(cup.indexOf('home')>=0){
                        var url=pathExView+homePort+'/index.html?reLogin';
                        var html='<li><a href="'+url+'" style="font-weight:bold;">&nbsp;智能家居系统平台</a></li>'
                        $('ul#platformUl').append(html);
                    }
                }else{
                    successBack(json);
    ...
    $(document).on(BJUI.eventType.initUI, function(e) {
        ...
        if ($.fn.iCheck) {
            ...
            $icheck.each(function(i) {
                ...
                $element
                    .on('ifCreated', function(e) {      // ifCreated：输入框被应用了iCheck样式
                    ...
                    .on('ifChanged', function() {   // ifChanged: 输入框的 checked 或 disabled 状态改变了
                        var num = $box.find('input[data-toggle="icheck"]:checked').size();
                        if($(this).context.checked){
                            if( num == $icheck.size()){
                                $(this).parents("div.bjui-datagrid").find("div.datagrid-box-h .datagrid-wrap-h table tr th input[data-toggle='icheck']").iCheck("check");
                            }
                        }else{
                            $(this).parents("div.bjui-datagrid").find("div.datagrid-box-h .datagrid-wrap-h table tr th input[data-toggle='icheck']").iCheck("uncheck");
                        }
            ...
            /* i-check check all */
            $icheck.filter('.checkboxCtrl').on('ifChanged', function(e) {     //表格里面的所有的多选框，不包括顶部的“全选”多选框
            ...
            $box.find('[data-toggle="ztree"]').each(function() {
                ...
                var setting = {
                    ...
                    callback: {
                        ...
                        beforeCheck   : op.beforeCheck  ? op.beforeCheck.toFunc()  : null,
                        ...
                        onExpand      : _onExpand,
                        onAsyncSuccess: op.onAsyncSuccess
                    },
                    data: {
                        simpleData: {
                            enable: op.simpleData || true,
                            idKey: op.idKey || "id",
                            pIdKey: op.pIdKey || "pId",
                            rootPId: op.rootPId || 0
                        },
                        key: {
                            title: op.title || '',
                            name : op.keyName || "name"
                        }
                    },
                    async: {
                        enable     : op.async || false,
                        url        : op.url || "",
                        contentType: op.contentType || "application/x-www-form-urlencoded",
                        dataType   : op.dataType  ||  "text",
                        autoParam  : op.autoParam || ["id=id"],
                        dataFilter : op.filter  || ""
                    }
                }
            ...
            $box.find('textarea[data-toggle="kindeditor"]').each(function() {
                ...
                KindEditor.create($editor, {
                    ...
                    afterChange              :function() {
                        $.CurrentDialog.find("#createDescnum").html(this.count('text'));
                    }
                })
            })
        }
        ...
        if (WebUploader) {
            var initWebUploader = function($element, index) {
                var old = $element.data('webuploader'), options = $element.data('options'), imgUploaded = $element.data('imgUploaded'), imgBasePath = $element.data('imgBasePath')
                ...
                if (options) {
                    ...
                    if (typeof options === 'object') {
                        ...
                        // 图片访问基地址
                            basePath = options.basePath || $element.data('basePath'),
                        ...
                        var addFile = function(file, isuploaded, _index) {
                            ...
                            var uploadedAttr = isuploaded && upunit === '张图片' ? ' style="cursor:pointer;" , width:800, height:500, mask:true, title:\'查看已上传图片\'}"' : '',
                            ...
                            if (!isuploaded) {
                                if (file.getStatus() === 'invalid') {
                                    ...
                                    uploader.makeThumb(file, function(error, src) {
                                        ...
                                        var img = $('<a href="' + src + '"><img id="upLoadImg" src="'+src+'"></a>')
                                ...
                                $imgWrap.empty().append('<a href="' + file.src + '"><img src="'+ file.src +'"></a>')
                        ...
                        if (typeof $element.data('uploaded') !== 'undefined')
                            ...
                            //options.uploaded = imgUploaded
                        if (options.uploaded) {
                            // 将已上传图片加到队列
                            addFile()

                            $wrap.find('li.uploaded').append('<input type="hidden" class="upload" name="'+ (options.upname || $element.data('name')) +'" value="'+($element.data('pictureUrl'))+'"><input type="hidden" class="upload" name="'+ ($element.data('id')) +'" value="'+($element.data('pictureId'))+'">')
                        ...
                        uploader.on('uploadSuccess', function(file, response) {
                            if (response[BJUI.keys.statusCode] != BJUI.statusCode.ok) {
                                ...
                                $li.find('input.upload').remove().end()
                                    .append('<input type="hidden" class="upload" name="'+ (options.upname || $element.data('name')) +'" value="'+ response.data.pictureUrl +'"><input type="hidden" class="upload" name="'+ ($element.data('id')) +'" value="'+ response.data.pictureId +'">')
                            }
                        })
                        // 上传失败
                        ...
                        uploader.onError = function(code) {
                            if (code === 'Q_EXCEED_NUM_LIMIT') {
                                BJUI.alertmsg('error', '只允许上传'+ options.fileNumLimit + upunit +'！')
                            } else if (code === 'Q_TYPE_DENIED') {
                                BJUI.alertmsg('error', '不支持的文件类型！')
                            } else if (code === 'F_EXCEED_SIZE') {
                                BJUI.alertmsg('error', '图片大小不能超过20M！')
                            } else if (code === 'F_DUPLICATE') {
                                BJUI.alertmsg('error', '已添加过该文件！')
                            } else if (code === 'F_NAME_LENGTH') {
                                BJUI.alertmsg('error', '图片名称不能超过120个字！')
                            } else {
                                BJUI.alertmsg('error', code)
                            }
                        }
